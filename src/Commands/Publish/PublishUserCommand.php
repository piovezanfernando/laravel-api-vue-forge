...
class PublishUserCommand extends PublishBaseCommand
{
     ...
     * @var string
     */
    protected $name = 'apiforge:publish:user';
    ...
    public function handle()
    {
        ...
        if (config('laravel_api_vue_forge.options.repository_pattern')) {
    ...
    {
        $viewsPath = config('laravel_api_vue_forge.path.views', resource_path('views/'));
        $templateType = config('laravel_api_vue_forge.templates', 'adminlte-templates');
    ...
    {
        $path = config('laravel_api_vue_forge.path.routes', base_path('routes/web.php'));

        $routeContents = g_filesystem()->getFile($path);
        $controllerNamespace = config('laravel_api_vue_forge.namespace.controller');
        $routeContents .= apiforge_nls(2)."Route::resource('users', '$controllerNamespace\UserController')->middleware('auth');";
    ...
    private function updateMenu()
    {
        ...
        $menuContents .= apiforge_nl().$usersMenuContent;
    ...
    private function publishUserController()
    {
        ...
        $controllerPath = config('laravel_api_vue_forge.path.controller', app_path('Http/Controllers/'));
        $controllerPath .= 'UserController.php';

        $controllerContents = view('laravel-api-vue-forge::scaffold.user.'.$name)->render();
    ...
    {
        $repositoryPath = config('laravel_api_vue_forge.path.repository', app_path('Repositories/'));
        ...
        $templateData = view('laravel-api-vue-forge::scaffold.user.user_repository')->render();
    ...
    {
        $requestPath = config('laravel_api_vue_forge.path.request', app_path('Http/Requests/'));
        ...
        $templateData = view('laravel-api-vue-forge::scaffold.user.create_user_request')->render();
    ...
    private function publishUpdateUserRequest()
    {
        ...
        $templateData = view('laravel-api-vue-forge::scaffold.user.update_user_request')->render();
        g_filesystem()->createFile($requestPath.$fileName, $templateData);
