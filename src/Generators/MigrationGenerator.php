...
class MigrationGenerator extends BaseGenerator
{
    public function __construct()
    {
        ...
        $this->path = config('laravel_api_vue_forge.path.migration', database_path('migrations/'));
    }

    public function generate()
    {
        $templateData = view('laravel-api-vue-forge::migration', $this->variables())->render();
        ...
        $this->config->commandComment(apiforge_nl().'Migration created: ');
    ...
    protected function generateFields(): string
    {
        ...
        if ($this->config->options->softDelete) {
            $softDeleteFieldName = config('laravel_api_vue_forge.timestamps.deleted_at', 'deleted_at');
        ...
        return implode(apiforge_nl_tab(1, 3), array_merge($fields, $foreignKeys));
    }
